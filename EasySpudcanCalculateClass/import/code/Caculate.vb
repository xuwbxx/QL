Imports System.Math
Imports Easy
Imports Easy.EasyTool
Imports EasyFiniteElement.EasyStructure
Imports Easy.Structure
Imports Easy.Structure.FiniteElement
Imports EasyFiniteElement
Imports EsAPI
...
Public Class SpudcanCaculate
    ...
    Public StructureKit As EasyStructureKit
    ...
    Public Function GetWarningAndErrorMessage() As String
        ...
        Return Message
    End Function
    Sub WriteResult(ByVal BoatID As Integer, AStructureKit As EasyStructureKit)
        Dim BoatsDataSet As DataSet = AStructureKit.GetData
    ...
    Function GetKu(SoilPhi As Double) As Double
        ...
        Return Ku
    End Function
    Sub CaculateByFiniteElement()
        On Error Resume Next
        'Dim MyDataSet As DataSet = AnApplication.GetStructureKit.GetData
        Dim WaterWeight As Double = 10
        Dim Row As DataRow, i As Integer
        Dim LegParameter As LegParameter = GetLegParameter() '腿参数
        Dim SpudcanParameter As SpudcanParameter = GetSpudcanParameter() '桩靴参数
        Dim CalculateParameter As CalculateParameter = GetCaculateParameter() '计算参数
        Dim Soils As List(Of SoilParameter) = GetSoils(CalculateParameter.UnderWaterPhiSubtractValue) '土层参数'WaterWeight
        Dim ComputeLevels As New List(Of Double)
        For Each LRow In MyDataSet.Tables("LS_CalculationLevels").Select("", "Level DESC")

            ComputeLevels.Add(Min(LRow("Level"), Soils(0).TopLevel - CalculateParameter.Hc2))

        Next
        'Dim ComputeLevels As List(Of Double) = GetComputeLevels(CalculateParameter.NCalculatePoint, Soils, Soils(0).TopLevel, CalculateParameter.DestinationLevel)
        '计算区域生成
        MyDataSet.Tables("LS_CalculationMaterials").Rows.Clear()
        'MyDataSet.Tables("LS_CalculationLevels").Rows.Clear()
        ...
        Dim SoilTopLevel As Double = Soils(0).TopLevel

        Dim LegTopLevel As Double = Soils(0).TopLevel + (SpudcanParameter.H1 + SpudcanParameter.H2 + SpudcanParameter.H3 + SpudcanParameter.H4 + 1) '桩腿的顶高层
        Dim SoilBottomLevel As Double = Min(Soils(Soils.Count - 1).TopLevel - 5, CalculateParameter.DestinationLevel - Max(LegTopLevel - CalculateParameter.DestinationLevel, 15))


        CalculateParameter.CaculateL = Max(Round(SoilTopLevel - SoilBottomLevel, 0), Max(CalculateParameter.CaculateL, Round（SpudcanParameter.B * 4, 0)))
        Dim eps As Double = 0.001
        Dim MinL As Double = CalculateParameter.MeshSize
        '计算每个深度下区域的形状
        'Dim StructureKit As EasyStructureKit = AnApplication.GetStructureKit
        Dim Parameter As EsApplicationParameter = StructureKit.GetApplicationParameter()
        '材料
        '腿的材料
        For i = 0 To ComputeLevels.Count - 1
            EsMessageReporter.ReportProgressFunction(100 * (i + 1) / ComputeLevels.Count)
            EsMessageReporter.ReportMessageFunction("有限元计算准备,计算高程=" & ComputeLevels(i), EsMessageType.Normal)
            Dim LegArea As EsTLArea = GetLegShapeArea(LegParameter, SpudcanParameter, LegTopLevel, ComputeLevels(i))  '获得地层形状
            Dim SoilLayerAreas As List(Of EsTLArea) = GetSoilLayerArea(SpudcanParameter.B * 0.5, ComputeLevels(i), Soils, CalculateParameter, SoilBottomLevel)
            Dim LegEdges As New List(Of EsTLEdge)
            Dim SoilEdges As New List(Of EsTLEdge)
            Dim LegAreas As New List(Of List(Of AreaEdge))
            Dim SoilAreas As New List(Of List(Of AreaEdge))
            '生成计算区域
            GenArea(i + 1, ComputeLevels(i), LegTopLevel, SoilBottomLevel, CalculateParameter.CaculateL, LegArea, SoilLayerAreas, LegAreas, SoilAreas, LegEdges, SoilEdges)
            EsMessageReporter.ReportMessageFunction("网格划分开始！计算高程=" & Round(ComputeLevels(i), 2), EsMessageType.Normal)
            '对计算区域进行网格划分
            GenMeshs(i + 1, SoilTopLevel, SoilBottomLevel, ComputeLevels(i), MinL, CalculateParameter.CaculateL, LegAreas, SoilAreas, LegEdges, SoilEdges)
            EsMessageReporter.ReportMessageFunction("网格划分结束！计算高程=" & Round(ComputeLevels(i), 2), EsMessageType.Normal)
            ''写入高程
            'Row = MyDataSet.Tables("LS_CalculationLevels").Rows.Add
            'Row("LevelID") = i + 1
            'Row("Level") = Round(ComputeLevels(i), 2)
        Next
        '设置压力标准值和拉力标准值，实际的结果就是这个值的系数

        Dim PressQ As Double = 1000
        Dim PullQ As Double = 1000
        '有限元计算按照轴对称计算，时候分三个阶段，
        '第一阶段计算自重下的应力场，按照塑性状态计算，桩侧与土通过接触连接，接触是光滑接触，桩底和桩靴面是完全接触，计算区域中心0点采用水平光滑支撑，计算区域右侧采用右侧采用无限单元支撑,无限元与单元之间采用点耦合，点只传递水平位移，不传递竖向位移，计算区域下采用无限单元支撑
        '第二阶段计算压力状态下的桩的极限抗压承载力，桩侧与土完全接触，桩底是完全接触，桩靴面是粘性接触，计算区域中心0点采用水平光滑支撑，计算区域右侧采用无限单元支撑，计算区域下采用无限单元支撑
        '第三阶段计算压力状态下的桩的极限抗拉承载力，桩侧与土完全接触，桩底是粘性接触，桩靴面是完全接触，计算区域中心0点采用水平光滑支撑，计算区域右侧采用无限单元支撑，计算区域下采用无限单元支撑
        '其中光滑接触不考虑桩土的侧摩擦力，不会在自重作用下载桩侧产生负摩擦力，应力场比较均匀，完全接触不考虑土的分离，粘贴接触考虑侧向和法向接触时候粘结力，
        '在采用Su指标时候，屈服面和Misses面一样，可能在很大的静水负压下，土依然不屈服，抗拉承载力会产生很大值，可以给屈服面拉力区域上加帽子
        '土在静水压压力作用下会
        '临界压力F=PressQ*临界荷载系数乘以桩顶面积


        '设置材料
#If Not WEB Then
        For i = 0 To ComputeLevels.Count - 1
            Dim BottomSoil As SoilParameter = GetSoil(ComputeLevels(i), Soils) '持力土层
            Dim SumH As Double = 0
            Dim AverageSoil As SoilParameter = GetAverageSoilValue(Soils, ComputeLevels(i), Soils(0).TopLevel, SumH) '平均土层
            '接触
            Dim ContactDictionaty As Dictionary(Of Integer, EsContact) = GetEdgeContact(AverageSoil, BottomSoil, ComputeLevels(i))
            Dim MaterialDictionaty As Dictionary(Of Integer, EsMaterial) = GetMaterials(Soils, CalculateParameter, AverageSoil, WaterWeight)
            EsMessageReporter.ReportProgressFunction(100 * (i + 1) / ComputeLevels.Count)
            EsMessageReporter.ReportMessageFunction("有限元计算中,计算高程=" & Round(ComputeLevels(i), 2), EsMessageType.Normal)
            Dim Edgerow As DataRow = MyDataSet.Tables("LS_CalculationEdges").Select("LevelID=" & (i + 1) & " and Type=6")(0)
            Dim LegTopEdgeID As Integer = Edgerow("EdgeID")
            Dim LegTopEdgeLength As Double = Sqrt((Edgerow("x1") - Edgerow("x2")) ^ 2 + (Edgerow("y1") - Edgerow("y2")) ^ 2)
            '有限元计算准备
            GetFiniteElementPrepare(i + 1, CalculateParameter, ComputeLevels(i), Soils(0).TopLevel, LegTopEdgeID, LegTopEdgeLength, PressQ, PullQ, MaterialDictionaty, ContactDictionaty)


            EsMessageReporter.ReportMessageFunction("有限元计算结束,计算高程=" & Round(ComputeLevels(i), 2), EsMessageType.Normal)
            '提取有限元计算结果
            '土与桩重合的体积,浮重度
            Dim SoilWeight As Double = (SpudcanParameter.Volume + LegParameter.Area * (Soils(0).TopLevel - ComputeLevels(i))) * AverageSoil.Weight

            GetFiniteElementResult(i + 1, BottomSoil, ComputeLevels(i), SpudcanParameter.Area, PressQ * LegTopEdgeLength ^ 2 * PI, PullQ * LegTopEdgeLength ^ 2 * PI, Max(SpudcanParameter.Weight - SoilWeight, 0))
...
#End If
    ...
    End Sub
    Sub GenArea(ByVal LevelID As Integer, LegDownLevel As Double, LegTopLevel As Double, SoilBottomLevel As Double, ByVal SoilWidth As Double, LegArea As EsTLArea, SoilLayerAreas As List(Of EsTLArea), LegAreaEdges As List(Of List(Of AreaEdge)), SoilAreaEdges As List(Of List(Of AreaEdge)), LegEdges As List(Of EsTLEdge), SoilEdges As List(Of EsTLEdge))
        Dim row As DataRow
        'Dim MyDataSet As DataSet = AnApplication.GetStructureKit.GetData
        Dim Areas As New List(Of EsTLArea)
        Dim LegAreas As New List(Of EsTLArea)
        Dim SoilAreas As New List(Of EsTLArea)
        Dim MergeredPoints As New List(Of EsTLPoint2D)
        Dim Edges As New List(Of EsTLEdge)

        Dim AreaEdgeID As Integer = 0
        GetMergerArea(LegArea, SoilLayerAreas, MergeredPoints, Areas)
        For Each P In MergeredPoints
            row = MyDataSet.Tables("LS_CalculationNodes").Rows.Add
            row("LevelID") = LevelID
            row("NodeID") = P.ID
            row("x") = P.x
            row("y") = P.y
        ...
        For Each Area In Areas
            Dim CurrentSoilAreas As New List(Of AreaEdge)
            Dim AreaEdges As List(Of EsTLEdge) = Area.GetEdges()
            For Each AreaEdge In AreaEdges
                AreaEdge.ID = 0
            Next
            If Area.Type = 1 Then
                Edges = LegEdges
            ...
            End If
            For Each AreaEdge In AreaEdges
                Dim ASoilArea As New AreaEdge()
                For Each Edge In Edges
                    If AreaEdge.IsSameGeometry(Edge) Then
                        If AreaEdge.P1.IsSame(Edge.P2) Then
        ...
        '写入腿的边界
        Dim LegDownB As Double = 0
        For Each Edge In LegEdges
            If Abs(Edge.P1.y - LegTopLevel) < 0.001 Then
                Edge.Type = 6
            End If
            '中心的边界
            If Abs(Edge.P1.x) < 0.001 And Abs(Edge.P2.x) < 0.001 Then
                Edge.Type = 1
            End If
            '桩底,
            Dim Vx, Vy As Double
            Vx = (Edge.P2.x - Edge.P1.x) / Edge.P2.GetDistance(Edge.P1)
            Vy = (Edge.P2.y - Edge.P1.y) / Edge.P2.GetDistance(Edge.P1)
            If Vx > 0.01 And Edge.Tag = 0 Then
                Edge.Type = 4
                LegDownLevel = Max(LegDownLevel, Edge.P1.y)
                LegDownLevel = Max(LegDownLevel, Edge.P2.y)
                LegDownB = Max(LegDownB, Edge.P1.x)
                LegDownB = Max(LegDownB, Edge.P2.x)
            End If


            '桩侧
            If Edge.Type = 0 And Edge.P2.x + 0.5 < Edge.P1.x Then
                Edge.Type = 7
            End If
        ...
        '写入土层边界
        For Each Edge In SoilEdges
            '中心的边界
            If Abs(Edge.P1.x) < 0.001 And Abs(Edge.P2.x) < 0.001 Then
                Edge.Type = 1
            End If
            '底部边界
            If Abs(Edge.P1.y - SoilBottomLevel) < 0.001 And Abs(Edge.P2.y - SoilBottomLevel) < 0.001 Then
                Edge.Type = 2
            End If
            '侧面的边界
            If Abs(Edge.P1.x - SoilWidth) < 0.001 And Abs(Edge.P2.x - SoilWidth) < 0.001 Then
                Edge.Type = 3
            End If
            '桩底,
            Dim Vx, Vy As Double
            Vx = (Edge.P2.x - Edge.P1.x) / Edge.P2.GetDistance(Edge.P1)
            Vy = (Edge.P2.y - Edge.P1.y) / Edge.P2.GetDistance(Edge.P1)
            If Vx < -0.001 And Edge.P1.y < LegDownLevel + 0.001 And Edge.P2.y < LegDownLevel + 0.001 And Edge.P1.x < LegDownB + 0.001 And Edge.P2.x < LegDownB + 0.001 Then
                Edge.Type = 4
            End If
            '桩侧
            For Each LegEdge In LegEdges
                If LegEdge.IsSameGeometry(Edge) Then
                    Edge.Type = LegEdge.Type
                End If
            Next
        Next

        Dim AllAreas As New List(Of EsTLArea)
        AllAreas.AddRange(LegAreas)
        AllAreas.AddRange(SoilAreas)
        Dim AllAreaEdges As New List(Of List(Of AreaEdge))
        AllAreaEdges.AddRange(LegAreaEdges)
        AllAreaEdges.AddRange(SoilAreaEdges)
        For i As Integer = 0 To AllAreas.Count - 1
            Dim Area As EsTLArea = AllAreas(i)
            row = MyDataSet.Tables("LS_CalculationAreas").Rows.Add
            row("LevelID") = LevelID
            row("AreaID") = i + 1
            row("x0") = Area.x
            row("y0") = Area.y
            row("MaterialID") = Area.GetPropertyID(1)
            If Area.y > LegDownLevel Then '在桩靴以上
                row("Location") = 1
            Else
                row("Location") = -1
            End If
        ...
        Next
    End Sub
    Public Shared Sub IsNodeExist(ByVal Nodes As List(Of EsTLPoint), ByRef Node As EsTLPoint, ByRef NodeID As Integer)
        For Each ANode In Nodes
            If ANode.IsSame(Node) Then
                Node.ID = ANode.ID
                Return
            End If
        Next
        ...
        Nodes.Add(Node)
    End Sub
    Sub GenContactorEdgeMesh(ByVal MinL As Double, ByVal Edge As EsTLEdge, ControlPoints As List(Of EsTLMeshControlPoint), ByRef NodeID As Integer, Nodes As List(Of EsTLPoint))
        Dim Dx, Dy, LocalL As Double
        Dim NControlPoint As Integer = ControlPoints.Count
        Dim ControlPointValues(1000, 5) As Double
        For i = 0 To ControlPoints.Count - 1
        ...
        Next
        Dx = (Edge.P2.x - Edge.P1.x) / Edge.P1.GetDistance(Edge.P2)
        Dy = (Edge.P2.y - Edge.P1.y) / Edge.P1.GetDistance(Edge.P2)
        IsNodeExist(Nodes, Edge.P1, NodeID)
        IsNodeExist(Nodes, Edge.P2, NodeID)
        Edge.MeshNodes.Add(Edge.P1)
        Do
            LocalL = GetLocalL(MinL, Edge.MeshNodes(Edge.MeshNodes.Count - 1).x, Edge.MeshNodes(Edge.MeshNodes.Count - 1).y, ControlPointValues, NControlPoint)
            If Sqrt((Edge.P2.x - Edge.MeshNodes(Edge.MeshNodes.Count - 1).x) ^ 2 + (Edge.P2.y - Edge.MeshNodes(Edge.MeshNodes.Count - 1).y) ^ 2) < LocalL * 1.5 Then
                Exit Do
            Else
                NodeID += 1
                Dim P As New EsTLPoint(NodeID, Round(Edge.MeshNodes(Edge.MeshNodes.Count - 1).x + Dx * LocalL, 2), Round(Edge.MeshNodes(Edge.MeshNodes.Count - 1).y + Dy * LocalL, 2), 0)
                Nodes.Add(P)
    ...
    End Sub
    Shared Function GetLocalL(MinL As Double, x As Double, y As Double, ControlPoints(,) As Double, NControlPoint As Integer) As Double
        GetLocalL = MinL
        Dim r As Single, xs As Single
        For i = 1 To NControlPoint
            r = Sqrt((ControlPoints(i, 0) - x) ^ 2 + (ControlPoints(i, 1) - y) ^ 2)

            xs = r / ControlPoints(i, 2)
            GetLocalL = Min(GetLocalL, ControlPoints(i, 3) * (1 - xs) + MinL * xs)

        Next i
    End Function
    Sub GenMeshs(ByVal LevelID As Integer, ByVal TopLevel As Double, ByVal BottomLevel As Double, ByVal LegBottomLevel As Double, ByVal MinL As Double, ByVal SoilWidth As Double, LegAreas As List(Of List(Of AreaEdge)), SoilAreas As List(Of List(Of AreaEdge)), LegEdges As List(Of EsTLEdge), SoilEdges As List(Of EsTLEdge))
        Dim row As DataRow
        'Dim MyDataSet As DataSet = AnApplication.GetStructureKit.GetData
        Dim ControlPoints As New List(Of EsTLMeshControlPoint)
        Dim Point As New EsTLMeshControlPoint

        '对进行网格划分
        Dim ToMeshLegEdges As New List(Of EsTLEdge), ToMeshLegAreas As New List(Of List(Of EsTLAreaEdge)), LegNodes As New List(Of EsTLPoint), LegTriMeshs As New List(Of List(Of EsTLTriMesh))
        Dim ToMeshSoilEdges As New List(Of EsTLEdge), ToMeshSoilAreas As New List(Of List(Of EsTLAreaEdge)), SoilNodes As New List(Of EsTLPoint), SoilTriMeshs As New List(Of List(Of EsTLTriMesh))
        '区域网格划分
        '腿的有限元划分
        For Each Edge In LegEdges
            ToMeshLegEdges.Add(Edge)
        Next
        For Each AreaEdge In LegAreas
            Dim MeshAreaEdge As New List(Of EsTLAreaEdge)
            ToMeshLegAreas.Add(MeshAreaEdge)
            For Each Edge In AreaEdge
                MeshAreaEdge.Add(Edge)
            Next
        Next
        Dim StartNodeID As Integer = 0, StartMeshID As Integer = 0
        '土层的有限元划分
        For Each Edge In SoilEdges
            ToMeshSoilEdges.Add(Edge)
        Next
        For Each AreaEdge In SoilAreas
            Dim MeshAreaEdge As New List(Of EsTLAreaEdge)
        ...
        For Each Edge In ToMeshLegEdges
            Point = New EsTLMeshControlPoint
            Point.x = Edge.P1.x
            Point.y = Edge.P1.y
            Point.MaxL = MinL
            Point.MinL = MinL * 0.25
            Point.Radius = MinL * 5
            Point.RadiusCoeff = 1
        ...
        'Point.MaxL = MinL * 8
        'Point.MinL = MinL * 0.25
        'Point.Radius = SoilWidth
        'Point.RadiusCoeff = 1
        'ControlPoints.Add(Point)
        Point = New EsTLMeshControlPoint
        Point.x = 0
        Point.y = LegBottomLevel
        Point.MaxL = MinL * 4
        Point.MinL = MinL * 0.25
        Point.Radius = SoilWidth
        Point.RadiusCoeff = 1
        ControlPoints.Add(Point)
        Dim ContactorEdges As New Dictionary(Of EsTLEdge, EsTLEdge)
        '设置腿与土层的接触
        Dim ContactorID As Integer = 1
        StartNodeID = 0
        For EdgeType As Integer = 4 To 7
            Dim Edge1s As New List(Of EsTLEdge)
            Dim Edge2s As New List(Of EsTLEdge)
            For Each Edge In LegEdges
                If Edge.Type = EdgeType Then
                    Edge1s.Add(Edge)
                End If
            Next
            For Each Edge In SoilEdges
                ...
                End If
            Next
            If Edge1s.Count > 0 And Edge2s.Count > 0 Then
                For Each edge1 In Edge1s
                    For Each edge2 In Edge2s
                        If edge1.IsSameGeometry(edge2) Then
                            '边上的节点组成接触
                            GenContactorEdgeMesh(MinL, edge1, ControlPoints, StartNodeID, LegNodes)
        ...
        EsTLDelaunayTriMeshGenerator.GenAreaMeshByEdge(MinL, ControlPoints, ToMeshLegEdges, ToMeshLegAreas, LegNodes, LegTriMeshs, StartNodeID, StartMeshID)
        For i As Integer = 0 To ContactorEdges.Count - 1
            Dim Edge1 As EsTLEdge = ContactorEdges.Keys(i)
            Dim Edge2 As EsTLEdge = ContactorEdges.Values(i)
            Dim EdgeNodes2 As New List(Of EsTLPoint)
            EdgeNodes2.AddRange(Edge1.MeshNodes)
            EdgeNodes2.Reverse()
            For Each APoint In EdgeNodes2
                Dim P As New EsTLPoint(0, APoint.x, APoint.y, 0)
                IsNodeExist(SoilNodes, P, StartNodeID)
                Edge2.MeshNodes.Add(P)
            Next
        Next

        EsTLDelaunayTriMeshGenerator.GenAreaMeshByEdge(MinL, ControlPoints, ToMeshSoilEdges, ToMeshSoilAreas, SoilNodes, SoilTriMeshs, StartNodeID, StartMeshID)
        Dim Nodes As New List(Of EsTLPoint)
        Nodes.AddRange(LegNodes)
        Nodes.AddRange(SoilNodes)

        Dim TriMeshs As New List(Of List(Of EsTLTriMesh))
        TriMeshs.AddRange(LegTriMeshs)
        TriMeshs.AddRange(SoilTriMeshs)
        Dim AllEdges As New List(Of EsTLEdge)
        AllEdges.AddRange(LegEdges)
        AllEdges.AddRange(SoilEdges)

        '写入网格划分结果
        '无限单元的划分
        Dim InfiniteMeshGeneratorParameters As New EsInfinite2DMeshGenParamter
        InfiniteMeshGeneratorParameters.MeshSize = 5
        ...
        For j = 0 To Nodes.Count - 1
            row = MyDataSet.Tables("LS_MeshNodes").Rows.Add
            row("LevelID") = LevelID
            row("NodeID") = Nodes(j).ID
            row("x") = Nodes(j).x
            row("y") = Nodes(j).y
            InfiniteMeshGeneratorParameters.NodeIndexMapToID.Add(Nodes(j).ID, New EsNode(Nodes(j).ID, 0, Nodes(j).x, Nodes(j).y, 0))
        Next
        '写入单元
        For j = 0 To TriMeshs.Count - 1
            Dim Meshs As List(Of EsTLTriMesh) = TriMeshs(j)
            For k = 0 To Meshs.Count - 1
                row = MyDataSet.Tables("LS_AreaMeshs").Rows.Add
                row("LevelID") = LevelID
                row("AreaID") = j + 1
                row("MeshID") = Meshs(k).ID
                row("N1") = Meshs(k).P1.ID
                row("N2") = Meshs(k).P2.ID
                row("N3") = Meshs(k).P3.ID
            Next
        Next

        Dim RightSideNodes As New Dictionary(Of Integer, EsTLPoint)
        For Each edge In AllEdges
            row = MyDataSet.Tables("LS_CalculationEdges").Rows.Add
            row("LevelID") = LevelID
            row("EdgeID") = edge.ID
            row("x1") = edge.P1.x
            row("y1") = edge.P1.y
            ...
            row("y2") = edge.P2.y
            row("Type") = edge.Type
            Select Case edge.Type
                Case 1
                    row("SupportID") = 100 '水平支撑
                Case 3
                    'row("ReleaseID") = 100 '右侧自由度释放，初始状态下与无限元连接时自由度释放
                    For Each Node In edge.MeshNodes
                        If Not RightSideNodes.ContainsKey(Node.ID) And Node.y > BottomLevel + 0.1 Then
        ...
        Next
        '利用边单元划分无限单元
        Dim Infinite2DMesh As New EsInfinite2DMesh()
        Dim InfiniteNodes As New List(Of EsNode)
        StartNodeID = Nodes.Count
        StartMeshID = 1
        For Each edge In SoilEdges
            If edge.Type = 2 Or edge.Type = 3 Then
                Dim EdgeMesh As New EsMesh
                EdgeMesh.LocalCoordinate = New EsAxis
                EdgeMesh.ID = edge.ID
        ...
        Next
        '通过边生成无限区域
        InfiniteMeshGeneratorParameters.GenMesh(StartNodeID, StartMeshID, Infinite2DMesh, InfiniteNodes)
        '增加边与无限元的耦合
        Dim CoupleNodeID As Integer = 0
        Dim NodeID As Integer = StartNodeID
        For Each RightSideNode In RightSideNodes
            row = MyDataSet.Tables("LS_MeshNodes").Rows.Add
            NodeID += 1
            row("LevelID") = LevelID
            row("NodeID") = NodeID
            ...
            For Each Mesh In Infinite2DMesh.Meshs
                For i = 0 To Mesh.NodeIDs.Count - 1
                    If Mesh.NodeIDs(i) = RightSideNode.Key Then
                        Mesh.NodeIDs(i) = NodeID
                    End If
                Next
            Next
            CoupleNodeID += 1
            row = MyDataSet.Tables("LS_CoupleNodes").Rows.Add
            row("LevelID") = LevelID
            row("CoupleNodeID") = CoupleNodeID
            row("N1") = RightSideNode.Key
            row("N2") = NodeID
            row("CoupleID") = 100
        Next

        For j = 0 To InfiniteNodes.Count - 1
            row = MyDataSet.Tables("LS_MeshNodes").Rows.Add
            row("LevelID") = LevelID
            row("NodeID") = InfiniteNodes(j).ID
            row("x") = InfiniteNodes(j).x
            row("y") = InfiniteNodes(j).y
        Next
        ...
        '写入无限区域
        For Each Mesh In Infinite2DMesh.Meshs
            row = MyDataSet.Tables("LS_InfiniteMeshs").Rows.Add
            row("LevelID") = LevelID
            row("MeshID") = Mesh.ID
        ...
        For i As Integer = 0 To ContactorEdges.Count - 1
            Dim Edge1 As EsTLEdge = ContactorEdges.Keys(i)
            Dim Edge2 As EsTLEdge = ContactorEdges.Values(i)
            row = MyDataSet.Tables("LS_Contactors").Rows.Add
            row("LevelID") = LevelID
            row("ID") = i + 1
            Select Case Edge1.Type
                Case 4
                    row("ContactorID") = 100 '"桩靴底完全接触" '第一阶段土与土的接触
                Case 5
                    row("ContactorID") = 101 '"桩侧完全接触" '第一阶段土与土的接触
                Case 7
                    row("ContactorID") = 102 '"桩靴面完全接触" '第一阶段土与土的接触
            End Select
            row("Edge1s") = Edge1.ID
            row("Edge2s") = Edge2.ID
            Dim Nodes1 As String = "", Nodes2 As String = ""
            For j As Integer = 0 To Edge1.MeshNodes.Count - 1
                If j = 0 Then
                ...
                End If
            Next
            For j As Integer = Edge2.MeshNodes.Count - 1 To 0 Step -1
                If j = Edge2.MeshNodes.Count - 1 Then
                    Nodes2 = Edge2.MeshNodes(j).ID.ToString
                Else
                    Nodes2 = Nodes2 & "," & Edge2.MeshNodes(j).ID.ToString
                End If
            Next
            Dim Vx As EsVector = Edge1.GetVector()
            Vx.NormalVector()
            Dim Axis As New EsAxis(Edge1.MeshNodes(0).x, Edge1.MeshNodes(0).y, 0, New EsVector(Vx.Vx, Vx.Vy, 0), New EsVector(-Vx.Vy, Vx.Vx, 0), New EsVector(0, 0, 1))
    ...
    Function GetMaterials(Soils As List(Of SoilParameter), CalculateParameter As CalculateParameter, AverageSoil As SoilParameter, WaterWeight As Double) As Dictionary(Of Integer, EsMaterial)
        '在计算抗压承载力时候让桩靴以上土为弹性，弹性模量取小值，桩靴以下土为弹塑性，当桩下压时候，土桩靴以上土不会破坏，由于桩靴上模量小，桩底下位移小，上底上土位移大，上部上部的土压力始终能够传递到桩靴，上部土不会束缚土的隆起
        '在计算抗拉承载力时候让桩靴以上土为塑性，弹性模量取正常值，桩靴以下土为弹性，当桩上拉时候，土桩靴以下土不会破坏，
        '根据以上规则，定义不同材料，在不同阶段进行置换计算
        Dim MaterialDictionaty As New Dictionary(Of Integer, EsMaterial)
        Dim AMaterial As New EsMaterial
        AMaterial.ID = 1000
        AMaterial.MaterialType = EsMaterialType.Steel
        AMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.LinearElastic
        AMaterial.Name = "桩腿"
        ...
        MaterialDictionaty.Add(AMaterial.ID, AMaterial)
        Dim AInifiniteMaterial As New EsMaterial

        AInifiniteMaterial.ID = 1001
        AInifiniteMaterial.MaterialType = EsMaterialType.Soil
        AInifiniteMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.LinearElastic
        AInifiniteMaterial.Name = "无限区域"
        AInifiniteMaterial.PlasticProperty.DruckerPragerProperty.ModeType = 1
        AInifiniteMaterial.PlasticProperty.DruckerPragerProperty.FrictionAngle = 0
        AInifiniteMaterial.PlasticProperty.DruckerPragerProperty.FlowAngle = 0
        AInifiniteMaterial.PlasticProperty.DruckerPragerProperty.Cohesion = 0
        AInifiniteMaterial.SoilProperty.Cohesion = 0
        AInifiniteMaterial.SoilProperty.SaturatedUnitWeight = AverageSoil.Weight + WaterWeight
        AInifiniteMaterial.UnitWeight = AverageSoil.Weight + WaterWeight
        AInifiniteMaterial.ElasticProperty.Type = 1
        AInifiniteMaterial.ElasticProperty.E = 10000
        AInifiniteMaterial.ElasticProperty.Mu = 0.4
        AInifiniteMaterial.ElasticProperty.IsAutoK0 = True
        MaterialDictionaty.Add(AInifiniteMaterial.ID, AInifiniteMaterial)

        '土壤材料
        Dim SumE As Double = 0, SumMu As Double = 0, NMaterial As Integer = 0, TopLevel As Double = Soils(0).TopLevel
        For Each SoilRow As DataRow In MyDataSet.Tables("LS_Soil").Rows
            For Each ASoil In Soils
                If ASoil.Name = SoilRow("Name") Then
            ...
            AMaterial.ID = SoilRow("ID") + 100
            AMaterial.MaterialType = EsMaterialType.Soil
            AMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.Plastic
            AMaterial.Name = SoilRow("Name")
            AMaterial.PlasticProperty.Type = 3
            AMaterial.PlasticProperty.DruckerPragerProperty.ModeType = CalculateParameter.DPType
            AMaterial.PlasticProperty.DruckerPragerProperty.FrictionAngle = If(SoilRow("Type") = SoilType.Sand, (SoilRow("UnderWaterPhi") - CalculateParameter.UnderWaterPhiSubtractValue), SoilRow("UnderWaterPhi")) 'SoilRow("UnderWaterphi")
            AMaterial.PlasticProperty.DruckerPragerProperty.FlowAngle = AMaterial.PlasticProperty.DruckerPragerProperty.FrictionAngle
            AMaterial.PlasticProperty.DruckerPragerProperty.Cohesion = SoilRow("Su0")
            AMaterial.PlasticProperty.DruckerPragerProperty.DCohesion = SoilRow("DSu")
            AMaterial.PlasticProperty.DruckerPragerProperty.Depth0 = TopLevel
            AMaterial.PlasticProperty.DruckerPragerProperty.LimitStresst = AMaterial.PlasticProperty.DruckerPragerProperty.Cohesion
            AMaterial.SoilProperty.Cohesion = AMaterial.PlasticProperty.DruckerPragerProperty.Cohesion

            AMaterial.SoilProperty.SaturatedUnitWeight = SoilRow("UnderWaterWeight") + WaterWeight '饱和重度
            AMaterial.UnitWeight = SoilRow("UnderWaterWeight") + WaterWeight '饱和重度
            AMaterial.ElasticProperty.E = SoilRow("E")
            AMaterial.ElasticProperty.Mu = SoilRow("Mu")
            AMaterial.ElasticProperty.IsAutoK0 = True
            MaterialDictionaty.Add(AMaterial.ID, AMaterial)

            AMaterial = New EsMaterial
            AMaterial.ID = SoilRow("ID") + 200
            AMaterial.MaterialType = EsMaterialType.Soil
            AMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.LinearElastic
            AMaterial.Name = SoilRow("Name") & "_弹性(桩靴以上)"
            AMaterial.SoilProperty.SaturatedUnitWeight = SoilRow("UnderWaterWeight") + WaterWeight '饱和重度
            AMaterial.UnitWeight = SoilRow("UnderWaterWeight") + WaterWeight '饱和重度
            ...
            AMaterial = New EsMaterial
            AMaterial.ID = SoilRow("ID") + 300
            AMaterial.MaterialType = EsMaterialType.Soil
            AMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.LinearElastic
            AMaterial.Name = SoilRow("Name") & "_弹性(桩靴以下)"
            ...
            AMaterial = New EsMaterial
            AMaterial.ID = SoilRow("ID") + 400
            AMaterial.MaterialType = EsMaterialType.Soil
            AMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.Plastic
            AMaterial.Name = SoilRow("Name") & "_折减"
            AMaterial.PlasticProperty.Type = 3
            AMaterial.PlasticProperty.DruckerPragerProperty.ModeType = CalculateParameter.DPType
            AMaterial.PlasticProperty.DruckerPragerProperty.FrictionAngle = Round(Atan(Tan(SoilRow("UnderWaterPhi") / 180 * PI) * SoilRow("OnLegStrenthengReduceCoeff")) * 180 / PI, 2)
            AMaterial.PlasticProperty.DruckerPragerProperty.FlowAngle = AMaterial.PlasticProperty.DruckerPragerProperty.FrictionAngle
            AMaterial.PlasticProperty.DruckerPragerProperty.Cohesion = SoilRow("Su0") * SoilRow("OnLegStrenthengReduceCoeff")
            AMaterial.PlasticProperty.DruckerPragerProperty.DCohesion = SoilRow("DSu") * SoilRow("OnLegStrenthengReduceCoeff")
            AMaterial.PlasticProperty.DruckerPragerProperty.Depth0 = TopLevel
            AMaterial.PlasticProperty.DruckerPragerProperty.LimitStresst = AMaterial.PlasticProperty.DruckerPragerProperty.Cohesion
            AMaterial.SoilProperty.Cohesion = AMaterial.PlasticProperty.DruckerPragerProperty.Cohesion

            AMaterial.SoilProperty.SaturatedUnitWeight = SoilRow("UnderWaterWeight") * SoilRow("OnLegWeightReduceCoeff") + WaterWeight '饱和重度
            AMaterial.UnitWeight = AMaterial.SoilProperty.SaturatedUnitWeight
            AMaterial.ElasticProperty.Type = 1
            AMaterial.ElasticProperty.E = SoilRow("E") * SoilRow("OnLegEReduceCoeff")
            AMaterial.ElasticProperty.Mu = SoilRow("mu") * SoilRow("OnLegMuReduceCoeff")
            AMaterial.ElasticProperty.IsAutoK0 = True
            MaterialDictionaty.Add(AMaterial.ID, AMaterial)
            ...
            AMaterial = New EsMaterial
            AMaterial.ID = SoilRow("ID") + 500
            AMaterial.MaterialType = EsMaterialType.Soil
            AMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.LinearElastic
            AMaterial.Name = SoilRow("Name") & "_折减弹性(桩靴以上)"

            AMaterial.SoilProperty.SaturatedUnitWeight = SoilRow("UnderWaterWeight") * SoilRow("OnLegWeightReduceCoeff") + WaterWeight '饱和重度
            AMaterial.UnitWeight = AMaterial.SoilProperty.SaturatedUnitWeight
            AMaterial.ElasticProperty.Type = 1
            AMaterial.ElasticProperty.E = 10
            AMaterial.ElasticProperty.Mu = SoilRow("mu") * SoilRow("OnLegMuReduceCoeff") * 0.01
            AMaterial.ElasticProperty.IsAutoK0 = True
            MaterialDictionaty.Add(AMaterial.ID, AMaterial)


            AMaterial = New EsMaterial
            AMaterial.ID = SoilRow("ID") + 600
            AMaterial.MaterialType = EsMaterialType.Soil
            AMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.LinearElastic
            AMaterial.Name = SoilRow("Name") & "_折减弹性(桩靴以下)"
            ...
            AMaterial = New EsMaterial
            AMaterial.ID = SoilRow("ID") + 700
            AMaterial.MaterialType = EsMaterialType.Soil
            AMaterial.ElasticPlasticType = EsMaterialElasticPlasticType.LinearElastic
            AMaterial.Name = SoilRow("Name") & "_置换桩区域"
            AMaterial.SoilProperty.Cohesion = AMaterial.PlasticProperty.DruckerPragerProperty.Cohesion
            AMaterial.SoilProperty.SaturatedUnitWeight = SoilRow("UnderWaterWeight") + WaterWeight '饱和重度
            AMaterial.UnitWeight = SoilRow("UnderWaterWeight") + WaterWeight '饱和重度
            AMaterial.ElasticProperty.Type = 1
        ...
        '无限单元采用平均土的参数
        If NMaterial > 0 Then
            AInifiniteMaterial.ElasticProperty.E = SumE / NMaterial * 10000
            AInifiniteMaterial.ElasticProperty.Mu = SumMu / NMaterial
        End If
        Return MaterialDictionaty
    End Function
    Function GetEdgeContact(AverageSoil As SoilParameter, BottomSoil As SoilParameter, BottomLevel As Double) As Dictionary(Of Integer, EsContact)
        Dim cohesionCoeff As Double = MyDataSet.Tables("LS_CalculationParameter").Rows(0).Item("cohesionCoeff")

        Dim ContactDictionaty As New Dictionary(Of Integer, EsContact)
        Dim AContact = New EsContact
        AContact.Name = "桩靴底完全接触" '第一阶段土与土的接触
        AContact.ID = 100
        AContact.Kt = 10000000000.0
        AContact.Kn = 10000000000.0
        AContact.Cn = 0
        AContact.Ct = 0
        AContact.FrictionCoeff = 0
        AContact.KeepContacting = True
        AContact.Deflection = 0.001
        AContact.InitialState = 0
        ContactDictionaty.Add(AContact.ID, AContact)

        AContact = New EsContact
        AContact.Name = "桩侧光滑接触" '第一阶段土与土的接触
        AContact.ID = 101
        AContact.Kt = 0
        AContact.Kn = 10000000000.0
        AContact.Cn = 0
        ...
        AContact = New EsContact
        AContact.Name = "桩靴面完全接触" '第一阶段土与土的接触
        AContact.ID = 102
        AContact.Kt = 10000000000.0
        AContact.Kn = 10000000000.0
        AContact.Cn = 0
        ...
        AContact = New EsContact
        AContact.Name = "桩靴底受压接触"
        AContact.ID = 103
        AContact.Kt = 10000000000.0
        AContact.Kn = 10000000000.0
        AContact.Ct = BottomSoil.GetSu(BottomLevel)
        AContact.Cn = AContact.Ct * cohesionCoeff

        AContact.FrictionCoeff = 0
        AContact.KeepContacting = False
        AContact.Deflection = 0.001
        AContact.InitialState = 0
        ContactDictionaty.Add(AContact.ID, AContact)

        AContact = New EsContact
        AContact.Name = "桩侧完全接触"
        AContact.ID = 104
        AContact.Kt = 10000000000.0
        AContact.Kn = 10000000000.0
        AContact.Ct = 0
        ...
        Return ContactDictionaty
    End Function
    Sub GetFiniteElementPrepare(ByVal LevelID As Integer, CalculateParameter As CalculateParameter, ByVal ComputeLevel As Double, ByVal TopLevel As Double, LegTopEdgeID As Integer, LegTopEdgeLength As Double, PressQ As Double, PullQ As Double, MaterialDictionaty As Dictionary(Of Integer, EsMaterial), ContactDictionaty As Dictionary(Of Integer, EsContact))
        Dim row, NewRow As DataRow
        'Dim MyDataSet As DataSet = AnApplication.GetStructureKit.GetData
        'Dim StructureKit As EasyStructureKit = AnApplication.GetStructureKit
        Dim Selfweightload As New EsStructureLoad
        Dim Pressload As New EsStructureLoad
        Dim Pullload As New EsStructureLoad
        '有限元计算
        '清除几何、材料、荷载，结果等
        Dim SelectDataValue As New EsSelectDataValue
        SelectDataValue.DefaultValue = True
        SelectDataValue.ModelGeometry = False
        SelectDataValue.StructureGeometry = True
        SelectDataValue.StructureStandarResult = True
        SelectDataValue.StructureCombinationResult = True
        SelectDataValue.StructureSectionValue = True
        StructureKit.StructureData.ClearData(SelectDataValue)
        '写入材料信息
        MyDataSet.Tables("Material").Clear() '分阶段属性
        For Each Material In MaterialDictionaty
            StructureKit.StructureData.AddMaterial(Material.Value)
            Dim FaceSection As New EsFaceSection
            FaceSection.ID = Material.Key
            FaceSection.MaterialID = Material.Key
            FaceSection.h1 = 1
            FaceSection.PlanStrainStressType = EsElementPlanStrainStressType.AxisSymetric '轴对称计算
            FaceSection.Name = Material.Value.Name
            StructureKit.StructureData.AddFaceSection(FaceSection)
        Next
        '写入接触
        For Each Contact In ContactDictionaty
            StructureKit.StructureData.AddContact(Contact.Value)
        Next
        '写入水平支撑
        Dim Support As New EsSupport
        Support.Name = "左侧水平支撑"
        Support.ID = 100
        Support.Ux = True
        StructureKit.StructureData.AddSupport(Support)

        Dim EdgeRelease As New EsEdgeRelease
        EdgeRelease.Name = "右侧边自由度释放"
        EdgeRelease.ID = 100
        EdgeRelease.Lx = True '边的局部坐标系
        StructureKit.StructureData.AddEdgeRelease(EdgeRelease)

        Dim Couple As New EsCouple
        Couple.Name = "右侧边X耦合"
        Couple.ID = 100
        Couple.Ux = True '边耦合
        Couple.Uy = False
        ...
        MyDataSet.Tables("PhaseSolveChangeToProperty").Clear() '分阶段属性


        NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
        NewRow("Phase") = 2
        NewRow("PropertyType") = 2 '耦合
        NewRow("PropertyID1") = 100 '与无限元连接时候，第一阶段边的，无限元产生约束，自重位移不正确)，第二阶段不释放
        NewRow("PropertyID2") = 101 '


        NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
        NewRow("Phase") = 3
        NewRow("PropertyType") = 2 '耦合
        NewRow("PropertyID1") = 100 '回去
        NewRow("PropertyID2") = 100 '
        ...
        '写入几何信息
        Dim Plates As New List(Of EsPlate)
        Dim MaterialID As Integer

        For Each row In MyDataSet.Tables("LS_CalculationAreas").Select("LevelID=" & LevelID)
            Dim Plate As New EsPlate
            Plate.ID = row("AreaID")
            Plate.Tag = row("AreaID")

            If row("BeforeMaterialID") <> 0 Then '初始位土材料，抗力计算为桩靴
                If (row("BeforeMaterialID") < 400) Then
                    '非折减
                    Plate.SectionID = row("BeforeMaterialID") + 600
                Else
                    '折减材料
                    Plate.SectionID = row("BeforeMaterialID") + 300
                End If
                NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
                '抗压计算
                NewRow("Phase") = 2
                NewRow("PropertyType") = 9 '材料
                NewRow("PropertyID1") = Plate.SectionID '第1阶段土
                NewRow("PropertyID2") = 1000 '第二阶段桩靴
                NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
                NewRow("Phase") = 3
                NewRow("PropertyType") = 9 '材料
                NewRow("PropertyID1") = Plate.SectionID '第1阶段土
                NewRow("PropertyID2") = Plate.SectionID '第1阶段土

                NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
                NewRow("Phase") = 4
                NewRow("PropertyType") = 9 '材料
                NewRow("PropertyID1") = Plate.SectionID '第1阶段土
                NewRow("PropertyID2") = 1000 '第4阶段桩靴
            Else
                If (row("MaterialID") = 1000) Then
                    MaterialID = 1000 '泥面以上桩部分
                Else
                    If (row("Location") > 0) Then
                        MaterialID = row("MaterialID") + 100 '桩靴以上弹性
                    Else
                        MaterialID = row("MaterialID") '桩靴以上塑性
                    End If
                    ...
                    NewRow("PropertyType") = 9 '材料
                    If (row("Location") > 0) Then
                        NewRow("PropertyID1") = MaterialID '桩靴以上弹性土
                        NewRow("PropertyID2") = MaterialID - 100 '桩靴以上塑性土
                    Else
                        NewRow("PropertyID1") = MaterialID '桩靴以下塑性土
                        NewRow("PropertyID2") = MaterialID + 200 '桩靴以下弹性土
                    End If

                End If
                Plate.SectionID = MaterialID
            End If
            Plate.ElementType = EsElementComputeType.Stress2DType
            Dim Bound As New EsPlaneBound
            Dim EdgeIDAndOrientations() As String = Split(row("Edges"), ";")
            For Each EdgeIDAndOrientation In EdgeIDAndOrientations
                Dim Edges() As String = Split(EdgeIDAndOrientation, ",")
                Dim EdgeID As Integer = Integer.Parse(Edges(0))
                Dim Orientation As Integer = Integer.Parse(Edges(1))
                For Each EdgeRow As DataRow In MyDataSet.Tables("LS_CalculationEdges").Select("LevelID=" & LevelID & " and EdgeID=" & EdgeID)
                    Dim LineEdge As EsLineEdge
                    LineEdge = New EsLineEdge
                    LineEdge.ID = EdgeRow("EdgeID")
                    LineEdge.Orientation = Orientation
                    LineEdge.Edge.x1 = EdgeRow("x1")
                    LineEdge.Edge.x2 = EdgeRow("x2")

                    LineEdge.Edge.y1 = EdgeRow("y1")
                    LineEdge.Edge.y2 = EdgeRow("y2")
                    LineEdge.Edge.z1 = 0
                    LineEdge.Edge.z2 = 0
                    LineEdge.SupportID = EdgeRow("SupportID")
                    LineEdge.ReleaseID = EdgeRow("ReleaseID")
        ...
        Next
        '几何形状
        Dim ShapeList As EsShapeList = EsPlate.GetPlaneBrepData(Plates)
        '写入单元信息
        For Each row In MyDataSet.Tables("LS_MeshNodes").Select("LevelID=" & LevelID) '节点
            Dim ANode As New EsNode
            ANode.ID = row("NodeID")
            ANode.Tag = row("NodeID")
            ANode.IsMeshNode = True
            ANode.x = row("x")
            ANode.y = row("y")
            If Abs(ANode.x) < 0.001 Then '位于中心轴上
                ANode.SupportID = 100 '水平支撑
            End If
            ShapeList.Nodes.Add(ANode)
        Next
        For Each row In MyDataSet.Tables("LS_CalculationEdges").Select("LevelID=" & LevelID) '边界单元
            Dim AEdgeMesh As New EsEdgeMesh
            AEdgeMesh.ID = row("EdgeID")

            AEdgeMesh.SupportID = row("SupportID")
            Dim Vx As New EsVector(row("x2") - row("x1"), row("y2") - row("y1"), 0)
            Dim Vy As New EsVector(row("y1") - row("y2"), row("x2") - row("x1"), 0)
            Dim Vz As New EsVector(0, 0, 1)
            Vx.NormalVector()
            Vy.NormalVector()
            LoadMesh(AEdgeMesh, row)
            For Each node In AEdgeMesh.Nodes
                AEdgeMesh.NodeVectorXs.Add(Vx)
                AEdgeMesh.NodeVectorYs.Add(Vy)
                AEdgeMesh.NodeVectorZs.Add(Vz)
            Next
            ShapeList.EdgeMeshs.Add(AEdgeMesh)
        Next

        For Each row In MyDataSet.Tables("LS_CoupleNodes").Select("LevelID=" & LevelID) '节点
            Dim ACoupleNode As New EsCoupleNode
            ACoupleNode.ID = row("CoupleNodeID")
            ACoupleNode.Tag = row("CoupleNodeID")
            ACoupleNode.N1 = row("N1")
            ACoupleNode.N2.Add(row("N2"))
            ACoupleNode.CoupleID = row("CoupleID")
            ShapeList.CoupleNodes.Add(ACoupleNode)
        Next



        For j = 0 To Plates.Count - 1
            Dim AFaceMesh As New EsFaceMesh
            AFaceMesh.ID = j + 1
            For Each row In MyDataSet.Tables("LS_AreaMeshs").Select("LevelID=" & LevelID & " and AreaID=" & (j + 1)) '面单元
                Dim AMesh As EsMesh = New EsMesh

                AMesh.Type = 10
                AMesh.ID = row("MeshID")
                AMesh.NodeIDs.Add(row("N1"))
                AMesh.NodeIDs.Add(row("N2"))
                AMesh.NodeIDs.Add(row("N3"))
                AFaceMesh.Meshs.Add(AMesh)
            Next

            AFaceMesh.Meshs(0).LocalCoordinate = New EsAxis
            ShapeList.FaceMeshs.Add(AFaceMesh)
        Next
        '无限单元
        Dim AInfinite2DMesh As New EsInfinite2DMesh()
        AInfinite2DMesh.ID = 1
        AInfinite2DMesh.ElementType = EsElementComputeType.Stress2DType
        AInfinite2DMesh.MaterialID = 1001
        AInfinite2DMesh.WithPhase = True
        '初始阶段不用无限元，第一阶段计算自重，第二阶段采用无限元
        AInfinite2DMesh.CreatePhase = 1
        AInfinite2DMesh.DemolishPhase = 100000
        AInfinite2DMesh.LocalCoordinate.SetString("0,0,0;1,0,0;0,1,0;0,0,1")
        For Each row In MyDataSet.Tables("LS_InfiniteMeshs").Select("LevelID=" & LevelID) '面单元
            Dim AMesh As EsMesh = New EsMesh
            AMesh.Type = EsMeshType.MeshTypeInfiniteQuad6

            AMesh.ID = row("MeshID")
            AMesh.NodeIDs.Add(row("N1"))
            AMesh.NodeIDs.Add(row("N2"))
            AMesh.NodeIDs.Add(row("N3"))
            AMesh.NodeIDs.Add(row("N4"))
            AMesh.NodeIDs.Add(row("N5"))
            AMesh.NodeIDs.Add(row("N6"))
            AInfinite2DMesh.Meshs.Add(AMesh)
        Next
        ShapeList.Infinite2DMeshs.Add(AInfinite2DMesh)
        '状态为桩靴完全只受压，底面面完全接触不判断
        '第一阶段，算自重，桩靴以上弹性，桩靴以下塑性

        '第二阶段算抗压，桩靴以上弹性，桩靴以下塑性

        '第三阶段，算自重，桩靴以上塑性，桩靴以下弹性


        '第四阶段，算抗拉，桩靴以上塑性，桩靴以下弹性



        NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
        NewRow("Phase") = 2
        NewRow("PropertyType") = 10 '桩靴上部接触
        NewRow("PropertyID1") = 102 '第一阶段接触,完全接触
        NewRow("PropertyID2") = 105 '第二阶段接触，只压不拉

        NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
        NewRow("Phase") = 2
        NewRow("PropertyType") = 10 '侧面接触
        NewRow("PropertyID1") = 101 '第一阶段接触,光滑接触
        NewRow("PropertyID2") = 104 '第二阶段接触,完全接触
        ...
        NewRow("PropertyType") = 10 '桩靴上部接触
        NewRow("PropertyID1") = 102 '第一阶段接触,完全接触
        NewRow("PropertyID2") = 102 '第一阶段接触,完全接触

        NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
        NewRow("Phase") = 3
        NewRow("PropertyType") = 10 '侧面接触
        NewRow("PropertyID1") = 101 '第一阶段接触,光滑接触
        NewRow("PropertyID2") = 101 '第一阶段接触,光滑接触




        '状态为桩靴底部只受压，顶面完全接触不判断
        NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
        NewRow("Phase") = 4
        NewRow("PropertyType") = 10 '桩靴底部接触
        NewRow("PropertyID1") = 100 '第一阶段接触,完全接触
        NewRow("PropertyID2") = 103 '第二阶段接触，只压不拉

        NewRow = MyDataSet.Tables("PhaseSolveChangeToProperty").Rows.Add
        NewRow("Phase") = 4
        NewRow("PropertyType") = 10 '桩靴上部接触
        NewRow("PropertyID1") = 102 '第一阶段接触,完全接触
        NewRow("PropertyID2") = 102 '第二阶段接触，完全接触




        Dim ContactorMeshID As Integer = 1
        For Each row In MyDataSet.Tables("LS_Contactors").Select("LevelID=" & LevelID) '面单元
            Dim AObjectContactor As New EsObjectContactor
            AObjectContactor.IsAutoContact = False
            AObjectContactor.ContactorType = EsObjectContactorType.EdgeToEdge
            AObjectContactor.ContactID = row("ContactorID")
            AObjectContactor.ID = row("ID")
            AObjectContactor.ObjectList1.AddRange(EsCommonFunction.GetIDList(row("Edge1s")))
            AObjectContactor.ObjectList2.AddRange(EsCommonFunction.GetIDList(row("Edge2s")))
            ShapeList.ObjectContactors.Add(AObjectContactor)
            Dim ContactorMesh As New EsObjectContactorMesh
            ShapeList.ObjectContactorMeshs.Add(ContactorMesh)
            ContactorMesh.ID = row("ID")
            ContactorMesh.ContactID = row("ContactorID")
            ContactorMesh.ElementType = EsElementComputeType.ContactorStress1DType
            ContactorMesh.ContactorType = EsObjectContactorType.EdgeToEdge





            Dim Nodes1 As List(Of Integer) = EsCommonFunction.GetIDList(row("Nodes1"))
            Dim Nodes2 As List(Of Integer) = EsCommonFunction.GetIDList(row("Nodes2"))
            Dim ContactorLocalCoordinate As New EsAxis(row("LocalCoordinate"))
            For i As Integer = 0 To Nodes1.Count - 2
                Dim AMesh As EsMesh = New EsMesh
                AMesh.ID = ContactorMeshID
                AMesh.Type = EsMeshType.MeshTypeContactorLine2
                AMesh.LocalCoordinate = ContactorLocalCoordinate
                AMesh.NodeIDs.Add(Nodes1(i))
                AMesh.NodeIDs.Add(Nodes1(i + 1))
                AMesh.NodeIDs.Add(Nodes2(i))
                AMesh.NodeIDs.Add(Nodes2(i + 1))
                ContactorMesh.Meshs.Add(AMesh)
                ContactorMeshID += 1
            Next
        Next
        '输入几何形状等数据
        ...
        '写入计算荷载
        row = MyDataSet.Tables("AnalyseSet").Rows(0)
        row("WeightDirection") = 0



        For Each LoadNonlinearrow As DataRow In MyDataSet.Tables("LoadNonlinearAnalyseSet").Rows
            For Each Loadrow As DataRow In MyDataSet.Tables("Load").Select("ID=" & LoadNonlinearrow("LoadID"))
        ...
        row = MyDataSet.Tables("Load").Rows.Add()
        row("ID") = 1
        row("Name") = "极限压力初始应力场"
        row("LoadType") = 104
        row("phase") = 1
        row("IsNonlinear") = True
        row("NNonlinearIteration") = 20
        row("NonlinearConvergentCriterion") = CalculateParameter.DCoeff
        ...
        row = MyDataSet.Tables("WaterLevelSet").Rows.Add()
        row("ID") = 1
        row("Name") = "设计水位"
        row("WaterLevel") = TopLevel + 50
        row("WaterLevelType") = 1
        MyDataSet.Tables("InitialSoilStress").Clear()

        row = MyDataSet.Tables("InitialSoilStress").Rows.Add()
        row("LoadID") = 1
        row("WaterLevelID") = 1
        ...
        row = MyDataSet.Tables("PlasticStableAnalyseSet").Rows.Add()
        row("LoadID") = 2
        row("InitialStressID") = 1
        row("MinDTime") = 0.01
        row("MaxTime") = 1000
        row("NNonlinearIteration") = 20
        row("NonlinearConvergentCriterion") = CalculateParameter.DCoeff
        ...
        row = MyDataSet.Tables("PlasticStableAnalyseSet").Rows.Add()
        row("LoadID") = 4
        row("InitialStressID") = 3
        row("MinDTime") = 0.01
        row("MaxTime") = 1000
        row("NNonlinearIteration") = 20
        row("NonlinearConvergentCriterion") = CalculateParameter.DCoeff
        Dim PressEdgeLoad As New EsLoadUniformOnEdge
        Dim PullEdgeLoad As New EsLoadUniformOnEdge

        PressEdgeLoad.ID = LegTopEdgeID
        PressEdgeLoad.IsRelative = True
        PressEdgeLoad.x1 = 0
        PressEdgeLoad.x2 = 1
        PressEdgeLoad.Qy1 = -PressQ
        PressEdgeLoad.Qy2 = -PressQ
        PressEdgeLoad.AddLoadType = EsAddLoadType.Linear


        PullEdgeLoad.ID = LegTopEdgeID
        PullEdgeLoad.IsRelative = True
        PullEdgeLoad.x1 = 0
        PullEdgeLoad.x2 = 1
        PullEdgeLoad.Qy1 = PullQ
        PullEdgeLoad.Qy2 = PullQ
        PullEdgeLoad.AddLoadType = EsAddLoadType.Linear

        Pressload.EdgeLoads.UniformLoads.Add(PressEdgeLoad)
        Pullload.EdgeLoads.UniformLoads.Add(PullEdgeLoad)
        Pressload.LoadID = 2
        Pullload.LoadID = 4
        StructureKit.StructureData.AddLoad(Pressload, True) '极限塑性荷载
        StructureKit.StructureData.AddLoad(Pullload, True) '极限塑性荷载

        For Each row In MyDataSet.Tables("LS_CalculationAreas").Select("LevelID=" & LevelID) '面单元
            Dim SelfWeightRow As DataRow = MyDataSet.Tables("LoadSelfWeightOnFace").Rows.Add
            SelfWeightRow("LoadID") = 1
            SelfWeightRow("FaceID") = row("AreaID")
            SelfWeightRow("WaterLevelID") = 1
            SelfWeightRow = MyDataSet.Tables("LoadSelfWeightOnFace").Rows.Add
            SelfWeightRow("LoadID") = 3
            SelfWeightRow("FaceID") = row("AreaID")
            SelfWeightRow("WaterLevelID") = 1
    ...
    End Sub
    Sub GetFiniteElementResult(ByVal LevelID As Integer, ByVal Soil As SoilParameter, ByVal ComputeLevel As Double, ByVal SpudcanA As Double, PressForce As Double, PullForce As Double, AddtionWeight As Double)
        '有限元计算结束
        'Dim MyDataSet As DataSet = AnApplication.GetStructureKit.GetData
        Dim Row As DataRow
        '提取结果
        For Each ResultRow In MyDataSet.Tables("PlasticStableResult").Select("LoadID=2") '极限压
            Row = MyDataSet.Tables("LS_PressResistanceResult").Rows.Add
            Row("DrillingID") = 1
            Row("ID") = LevelID
            Row("Level") = ComputeLevel
            Row("SoilID") = Soil.SoilID
            Row("IsSand") = Soil.SoilType = SoilType.Sand
            Row("QvP") = Round((ResultRow("StableCoeff") * PressForce - AddtionWeight) / SpudcanA, 2) '轴对称
            Row("Qv") = Round(ResultRow("StableCoeff") * PressForce - AddtionWeight, 2) '轴对称
        Next
        If LevelID > 1 Then
            For Each ResultRow In MyDataSet.Tables("PlasticStableResult").Select("LoadID=4") '极限拉
                Row = MyDataSet.Tables("LS_PullResistanceResult").Rows.Add
                Row("DrillingID") = 1
                Row("ID") = LevelID
                Row("Level") = ComputeLevel
                Row("SoilID") = Soil.SoilID
                Row("Qu") = Round(ResultRow("StableCoeff") * PullForce + AddtionWeight, 2) '轴对称
            Next
        Else
            Row = MyDataSet.Tables("LS_PullResistanceResult").Rows.Add
            Row("DrillingID") = 1
            ...
            Row("Level") = ComputeLevel
            Row("SoilID") = Soil.SoilID
            Row("Qu") = Round(AddtionWeight, 2)
        End If
        '提取节点位移
        For Each ResultRow In MyDataSet.Tables("ResultOfNodeDisplacement").Rows
            '.Add("LevelID", System.Type.GetType("System.Int32"))
            '.Add("LoadID", System.Type.GetType("System.Int32"))
            '.Add("NodeID", System.Type.GetType("System.Int32"))
            '.Add("Ux", System.Type.GetType("System.Double"))
            '.Add("Uy", System.Type.GetType("System.Double"))
            Row = MyDataSet.Tables("LS_ResultOfNodeDisplacement").Rows.Add
            Row("LevelID") = LevelID
        ...
        Next
        '提取区域应力
        For Each ResultRow In MyDataSet.Tables("ResultOfFace").Rows
            Row = MyDataSet.Tables("LS_ResultOfFace").Rows.Add
            Row("LevelID") = LevelID
            Row("LoadID") = ResultRow("LoadID")
        ...
        Next
    End Sub
    Sub LoadMesh(Amesh As EsMesh, item As DataRow)
        If item("MeshNodes") <> "" Then
            Dim Nodes() As String = Split(item("MeshNodes"), ",")
            For i As Integer = 0 To Nodes.Length - 1
                Amesh.NodeIDs.Add(Integer.Parse(Nodes(i)))
            Next i
        End If
    End Sub
    Function GetNodeString(ByVal Points As List(Of EsTLPoint)) As String
        Dim P As String = ""
        For i As Integer = 0 To Points.Count - 1
            If i = 0 Then
                P = Points(i).ID.ToString
            Else
                P = P & "," & Points(i).ID.ToString
            End If
        Next
        Return P
    End Function
    Function GetNodeString(ByVal Points As List(Of EsTLPoint2D)) As String
        Dim P As String = ""
        For i As Integer = 0 To Points.Count - 1
            If i = 0 Then
                P = Points(i).ID.ToString
        ...
        Return P
    End Function
    Function GetEdgeString(ByVal Edges As List(Of AreaEdge)) As String
        Dim P As String = ""
        For i As Integer = 0 To Edges.Count - 1
            If i = 0 Then
                P = Edges(i).Edge.ID.ToString & "," & Edges(i).Orientation.ToString
            Else
                P = P & ";" & Edges(i).Edge.ID.ToString & "," & Edges(i).Orientation.ToString
            End If
        Next
        Return P
    End Function
    Function GetLegShapeArea(ByVal LegParameter As LegParameter, SpudcanParameter As SpudcanParameter, CaculateTopLevel As Double, ByVal CaculateBottomLevel As Double) As EsTLArea '获得地层形状
        Dim LegShape As New LegShape
        LegShape.B2 = Round(LegParameter.Diameter, 2)
        LegShape.B1 = Round(Sqrt(SpudcanParameter.Area / Math.PI) * 2, 2)
        ...
        Dim Area As EsTLArea = LegShape.GetArea(CaculateTopLevel, CaculateBottomLevel)
        Dim MaterialID As Integer = 1000
        Area.Propertys.Add(New EsTLProperty(1, MaterialID))
        Return Area
    End Function

    Function GetSoilLayerArea(LegWidth As Double, LegBottomLevel As Double, Soils As List(Of SoilParameter), CalculateParameter As CalculateParameter, ByVal BottomLevel As Double) As List(Of EsTLArea) '获得土层的计算区域
        Dim SoilAreas As New List(Of EsTLArea)
        Dim CopySoils As New List(Of SoilParameter)
        Dim HCLevel As Double = Soils(0).TopLevel - CalculateParameter.Hc2 '洞口顶高程
        '通过桩底高程，插入一个高程
        For i As Integer = 0 To Soils.Count - 1
            Dim ASoilParameter As SoilParameter = Soils(i).GetCopy
            CopySoils.Add(ASoilParameter)
            If LegBottomLevel > ASoilParameter.BottomLevel And LegBottomLevel < ASoilParameter.TopLevel Then
            ...
            End If
            '插入洞口顶部高程
            If CalculateParameter.Hc2 > 0 And HCLevel > ASoilParameter.BottomLevel And HCLevel < ASoilParameter.TopLevel Then
        ...
        Next
        '桩底高程以上，桩靴宽度范围内换土
        For i As Integer = 0 To CopySoils.Count - 1
            Dim Soil As SoilParameter = CopySoils(i)
            If Soil.TopLevel > BottomLevel Then
                Dim Area As New EsTLArea
                Dim MaterialID As Integer = Soil.SoilID + 100
                Area.Propertys.Add(New EsTLProperty(1, MaterialID))
                SoilAreas.Add(Area)
                Area.Points.Add(New EsTLPoint2D(0, LegWidth, If(i = CopySoils.Count - 1, BottomLevel, Soil.BottomLevel)))
                Area.Points.Add(New EsTLPoint2D(0, CalculateParameter.CaculateL, If(i = CopySoils.Count - 1, BottomLevel, Soil.BottomLevel)))
                ...
                Area.Type = 2 '表示土层区域
                Area.Area = EsTLGeometry.ComputeArea(Area.Points, Area.x, Area.y)

                If (Soil.TopLevel + Soil.BottomLevel) * 0.5 < HCLevel Then '洞口以下
                    Area = New EsTLArea
                    MaterialID = IIf((Soil.TopLevel + Soil.BottomLevel) * 0.5 > LegBottomLevel, Soil.SoilID + 400, Soil.SoilID + 100)
                    Area.Propertys.Add(New EsTLProperty(1, MaterialID))
                    SoilAreas.Add(Area)
                    Area.Points.Add(New EsTLPoint2D(0, 0, If(i = CopySoils.Count - 1, BottomLevel, Soil.BottomLevel)))
                    Area.Points.Add(New EsTLPoint2D(0, LegWidth, If(i = CopySoils.Count - 1, BottomLevel, Soil.BottomLevel)))
                    Area.Points.Add(New EsTLPoint2D(0, LegWidth, Soil.TopLevel))
                    Area.Points.Add(New EsTLPoint2D(0, 0, Soil.TopLevel))
                    Area.Type = 2 '表示土层区域
                    Area.Area = EsTLGeometry.ComputeArea(Area.Points, Area.x, Area.y)
                End If
        ...
        Return SoilAreas
    End Function
    Sub GetMergerArea(LegShapeArea As EsTLArea, SoilLayerAreas As List(Of EsTLArea), MergeredPoints As List(Of EsTLPoint2D), Areas As List(Of EsTLArea)) '获得合并后的区域
        Dim AreaID As Integer = 1
        Dim PointID As Integer = 1
        LegShapeArea.ID = 1
        LegShapeArea.Type = 1
        Areas.Clear()
        MergeredPoints.Clear()
        Dim IsExisPoint As Boolean
        Dim AddAreas As List(Of EsTLArea) = EsTLArea.GetAddAreas(SoilLayerAreas, LegShapeArea)
        For Each AddArea In AddAreas
            AddArea.ID = AreaID
            AreaID += 1
            For Each P1 In AddArea.Points
                IsExisPoint = False
                For Each P2 In MergeredPoints
                    If P1.IsSame(P2) Then
                        IsExisPoint = True
                        Exit For
                    End If
                Next
                If IsExisPoint = False Then
                    MergeredPoints.Add(P1)
                    P1.ID = PointID
                    PointID = PointID + 1
                End If
            Next
...
Public Class AreaEdge
    Inherits EsTLAreaEdge
    Public ID As Integer
    Public N As Integer
    Sub New(ByVal ID As Integer, ByVal Edge As EsTLEdge, ByVal Orientation As Integer)
        Me.ID = ID
        Me.Edge = Edge
...
Class LegShape
    ...
    Sub New()
        ...
        H4 = 0
    End Sub
    Function GetArea(ByVal TopLevel As Double, ByVal BottomLevel As Double) As EsTLArea
        '一半区域
        Dim SoilArea As New EsTLArea
        ...
        'SoilArea.Points.Add(New EsTLPoint2D(0, B1 * 0.5, BottomLevel + H1 + H2))
        'SoilArea.Points.Add(New EsTLPoint2D(0, B1 * 0.5, BottomLevel + H1 + H2 + H3))
        'SoilArea.Points.Add(New EsTLPoint2D(0, B2 * 0.5, BottomLevel + H1 + H2 + H3 + H4))
