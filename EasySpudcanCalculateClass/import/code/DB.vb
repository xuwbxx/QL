Imports System.Windows.Forms, EsAPI, Easy.EasyPlot
Imports Easy.EasyTool
Imports System.Data.SqlClient
Imports System.IO, System.Math
Imports Easy, EasyFiniteElement.EasyStructure
Imports System.Windows
Imports System.Data.SqlTypes, System.Security.Cryptography, System.Text
...
Public Class SpudcanDB
    ...
    Sub New(StructureKit As EasyStructureKit, CreateTable As Boolean, Optional ByVal Boats As Boolean = True)
        Me.MydataSet = StructureKit.StructureData.GetData
        Me.StructureKit = StructureKit
        If CreateTable Then
            CreateDatabase(Me.StructureKit, Boats)
        End If
    End Sub
    ...
    End Function
    Public Sub CreateDatabase(ByRef AnEasyStructureKit As EasyStructureKit, Optional ByVal Boats As Boolean = True) 'Boats=0时获得单艘船和多艘船数据库，Boats=1时获得多艘船数据库
        MydataSet = AnEasyStructureKit.GetData
        ...
        AnEasyStructureKit.GetData.AcceptChanges()
    End Sub
    ...
    Sub GetBoatsDataFromABoat()
        ...
        End If
    End Sub
    Function GetBoatAppDic() As Dictionary(Of Integer, EasyStructureKit)
        Dim BoatsDataSet As DataSet = MydataSet
        Dim BoatAppDic As New Dictionary(Of Integer, EasyStructureKit)
        Dim ABoatDataSet As DataSet
        For Each Brow In BoatsDataSet.Tables("LS_Boat").Rows
            Dim StructureKit As New EasyStructureKit(System.Windows.Forms.Application.StartupPath)
            'Dim AnApplication = New EsApplication(StructureKit)
            BoatAppDic.Add(Brow("ID"), StructureKit)
            CreateDatabase(StructureKit, False)
            ABoatDataSet = StructureKit.GetData
    ...
    End Sub
    '获得结果的精度格式
    Friend Function GetFormat(ByVal ACB_DecimalDigits As System.Windows.Forms.ComboBox) As String
        Dim Format As String = "0."
        For i As Integer = 0 To ACB_DecimalDigits.SelectedIndex
            Format &= "0"
        Next
        Return Format
    End Function
    ...
    Private Shared Function GetLayerLevels(ByVal MyDataSet As DataSet, DrillingID As Integer, BoatID As Integer) As List(Of Double)
        ...
        Return MustShowV2s
    End Function
    Shared Function DrawPressCurve(MyDataSet As DataSet, PressCurveTable As EsPLCurveTable, LimitValue As Double, Width As Double, Height As Double, Optional XSep As Double = 1, Optional YSep As Double = 1, Optional DrillingID As Integer = 1, Optional BoatID As Integer = -1, Optional UseMetaFile As Boolean = True, Optional ResultRangeMultiple As Double = 2, Optional LimitValueShowMultiple As Double() = Nothing) As Drawing.Image
    ...
    End Function
    Public Sub RedrawAllDesignStructure()
        Dim ShapeList As New EsShapeList
        'DrawLeg(ShapeList)
        'DrawSoilLayers(ShapeList)
        Dim SelectDataValue As New EsSelectDataValue
        SelectDataValue.DefaultValue = False
        SelectDataValue.ModelGeometry = True
        SelectDataValue.StructureGeometry = False
        SelectDataValue.StructureStandarResult = False
        SelectDataValue.StructureCombinationResult = False
    ...
    Shared Function GetDBTabInputInformation(ByVal TheGDV As DataGridView, ByVal Title As String, ByVal ValueDic As Dictionary(Of String, String()), ByVal ShowMsgBox As Boolean)
        Dim InformationString As String = ""
        For i = 0 To TheGDV.Rows.Count - 1
            For j = 0 To TheGDV.Columns.Count - 1
                If TheGDV.Columns(j).Visible = True Then
                    If IsError(TheGDV.Rows(i).Cells(j).Value) OrElse IsNothing(TheGDV.Rows(i).Cells(j).Value) OrElse IsDBNull(TheGDV.Rows(i).Cells(j).Value) OrElse TheGDV.Rows(i).Cells(j).Value.ToString = "" Then
                        InformationString &= "第" & i + 1 & "行，" & TheGDV.Columns(j).HeaderText.Replace(Chr(13), "") & "列" & Chr(13)
                    Else
                        For Each Condition In ValueDic.Keys
                            For Each HeaderName In ValueDic(Condition)
                                If TheGDV.Columns(j).Name.ToUpper = HeaderName.ToUpper Then
                                    If If(Condition = "<=0", TheGDV.Rows(i).Cells(j).Value <= 0, If(Condition = "<0", TheGDV.Rows(i).Cells(j).Value < 0, TheGDV.Rows(i).Cells(j).Value = ",")) Then
                                        InformationString &= "第" & i + 1 & "行，" & TheGDV.Columns(j).HeaderText.Replace(Chr(13), "") & "列" & Chr(13)
                                    End If
                                End If
                            Next
            ...
            Next
        Next
        If InformationString <> "" Then
            InformationString = Title & If(ShowMsgBox, "表格内数值未输入或输入格式错误，具体位置如下：", "表格：") & Chr(13) & InformationString
            If ShowMsgBox Then MsgBox(InformationString, MsgBoxStyle.Exclamation, "输入警告")
        End If
        Return InformationString
    End Function
    Shared Function GetTextAndComboBoxInputInformation(ByVal InputControl() As Control, ByVal ShowString() As String) As String
        Dim InformationString As String = ""
        For i = 0 To InputControl.Count - 1
            Dim AControl As Control = InputControl(i)
            If TypeOf (AControl) Is TextBox Then
                Dim ATextBox As TextBox = CType(AControl, TextBox)
                If ATextBox.Enabled = True AndAlso (IsError(ATextBox.Text) Or IsDBNull(ATextBox.Text) Or ATextBox.Text = "") Then
                    InformationString &= ShowString(i).Replace("：", "") & Chr(13)
                End If
            End If
            If TypeOf (AControl) Is ComboBox Then
                Dim AComboBox As ComboBox = CType(AControl, ComboBox)
                If AComboBox.Enabled = True AndAlso (IsError(AComboBox.Text) Or IsDBNull(AComboBox.Text) Or AComboBox.Text = "") Then
                    InformationString &= ShowString(i).Replace("：", "") & Chr(13)
                End If
            End If
        Next
        If InformationString <> "" Then
            InformationString = "界面内数值未输入或输入格式错误，参数具体名称如下：" & Chr(13) & InformationString
            MsgBox(InformationString, MsgBoxStyle.Exclamation, "输入警告")
        End If
        Return InformationString
    End Function
End Class
...
Public Class EasyOnLine
    Public Shared UserLogined As Boolean = False
    Public Shared OnLineConnectString As String
    Public Shared ServerName As String
    Public Shared ServerLoginName As String
    Public Shared ServerLoginPassWord As String
    Public Shared DataBaseName As String
    Public Shared CurrentUserID As Integer
    Public Shared CurrentUserName As String
    Public Shared CurrentPassword As String
    Public Shared Function GetLogInStatus()
        Dim TempSHA As SHA256 = SHA256.Create()
        Dim TempByte() As Byte = TempSHA.ComputeHash(Encoding.UTF8.GetBytes(System.Windows.Forms.Application.StartupPath & Format(Date.Now, "MM-dd-yyyy")))
        Return Convert.ToBase64String(TempByte)
    End Function
    Public Shared Sub GetConnectString(ApplicationPath As String)
        Dim SettingDataset As New DataSet
        If Dir(ApplicationPath & "\Setting\LegAndSpudcanSetting.xml") <> "" Then
            Dim ATable As New DataTable("DatabaseSetting")
            With ATable.Columns
                .Add("DatabaseName", System.Type.GetType("System.String"))
                .Add("ServerName", System.Type.GetType("System.String"))
                .Add("LoginName", System.Type.GetType("System.String"))
                .Add("PassWord", System.Type.GetType("System.String"))
            End With

            SettingDataset.Tables.Add(ATable)
            ATable = New DataTable("UserSetting")
            With ATable.Columns
                .Add("UserID", System.Type.GetType("System.String"))
                .Add("UserName", System.Type.GetType("System.String"))
                .Add("PassWord", System.Type.GetType("System.String"))
            End With
            SettingDataset.Tables.Add(ATable)
            SettingDataset.ReadXml(ApplicationPath & "\Setting\LegAndSpudcanSetting.xml")
            If SettingDataset.Tables("UserSetting").Rows.Count Then
                Dim Row As DataRow = SettingDataset.Tables("DatabaseSetting").Rows(0)
                Row = SettingDataset.Tables("UserSetting").Rows(0)
                CurrentUserID = Row("UserID")
                CurrentUserName = Row("UserName")
                CurrentPassword = Row("PassWord")
            End If
            If SettingDataset.Tables("DatabaseSetting").Rows.Count Then
                Dim Row As DataRow = SettingDataset.Tables("DatabaseSetting").Rows(0)
                ServerName = Row("ServerName")
                ServerLoginName = Row("LoginName")
                ServerLoginPassWord = Row("PassWord")
                DataBaseName = Row("DatabaseName")
                OnLineConnectString = "Data Source =" & Row("ServerName") & ";Integrated Security=false;uid=" & Row("LoginName") & ";pwd=" & Row("PassWord") & ";Initial Catalog=" & Row("DatabaseName") & ";MultipleActiveResultSets=true"
            End If
        End If
    End Sub
    Shared Sub saveSetting(ApplicationPath As String)
        Dim row As DataRow
        Dim SettingDataset As New DataSet
        Dim ATable As New DataTable("DatabaseSetting")
        With ATable.Columns
        ...
        row = SettingDataset.Tables("DatabaseSetting").Rows.Add
        row("ServerName") = ServerName
        row("DatabaseName") = DataBaseName
        row("LoginName") = ServerLoginName
        row("PassWord") = ServerLoginPassWord


        row = SettingDataset.Tables("UserSetting").Rows.Add
        row("UserID") = CurrentUserID
        row("UserName") = CurrentUserName
        row("PassWord") = CurrentPassword

        If Dir(ApplicationPath & "\Setting\LegAndSpudcanSetting.xml") <> "" Then
            Kill(ApplicationPath & "\Setting\LegAndSpudcanSetting.xml")
        End If
        SettingDataset.WriteXml(ApplicationPath & "\Setting\LegAndSpudcanSetting.xml")
    End Sub
    Public Shared Function CheckUser(ByVal UserName As String, Password As String, ByRef ErrorString As String) As Boolean
        Dim CNN As SqlConnection
        Dim ATable As DataTable = Nothing
        CNN = New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        UserLogined = False
        If CNN.State <> ConnectionState.Closed Then
            Dim DataAdapter As New SqlDataAdapter("Select * from Users where Valid=1 and LoginName='" & UserName & "'" & " and Password='" & Password & "'", CNN)
            ATable = New DataTable
            DataAdapter.Fill(ATable)
            If ATable.Rows.Count > 0 Then
                CurrentUserID = ATable.Rows(0)("ID")
                CurrentUserName = UserName
                CurrentPassword = Password
                UserLogined = True
                Return True
            Else
                ErrorString = ("用户名或密码不对，或用户失效")
            End If
            CNN.Close()
        Else
            ErrorString = ("连接不到服务器")
        ...
        Return False
    End Function
    Public Shared Function GetDataTable(ByVal SqlString As String) As DataTable
        On Error Resume Next
        Dim CNN As SqlConnection
        Dim ATable As DataTable = Nothing
        CNN = New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        If CNN.State <> ConnectionState.Closed Then
            Dim DataAdapter As New SqlDataAdapter(SqlString, CNN)
            ATable = New DataTable
            DataAdapter.Fill(ATable)
            CNN.Close()
        End If
        Return ATable
    End Function
    Public Shared Sub GetSearchTreeRoot(ByVal TreeView As Forms.TreeView, ByVal Key As String)
        TreeView.Nodes.Clear()
        Dim DirectoryTable As DataTable = GetDataTable("Select * from Directorys where DirectoryName like '%" & Key & "%'")
        If Not IsNothing(DirectoryTable) Then
            For Each row As DataRow In DirectoryTable.Rows
                Dim ANode As New TreeNode
                ANode.Name = "Directory." & row("ID")
                ANode.Text = row("DirectoryName")
                ANode.Tag = "Directory"
                ANode.ImageIndex = 1 '文件夹
                ANode.SelectedImageIndex = 2 '文件夹
                TreeView.Nodes.Add(ANode)
            Next
            Dim FileTable As DataTable = GetDataTable("Select * from Files where FileName like '%" & Key & "%'")
            For Each row As DataRow In FileTable.Rows
                Dim ANode As New TreeNode
                ANode.Name = "FileName." & row("ID")
                ANode.Text = row("FileName")
                ANode.Tag = "File"
                ANode.ImageIndex = 3 '文件
                ANode.SelectedImageIndex = 3 '文件
                TreeView.Nodes.Add(ANode)
            Next
        End If
    End Sub

    Public Shared Sub GetTreeRoot(ByVal UserID As Integer, ByVal TreeView As Forms.TreeView)
        Dim DirectoryTable As DataTable = GetDataTable("Select * from Users where ID=" & UserID & " or ID=0 order by ID")
        If Not IsNothing(DirectoryTable) Then
            TreeView.Nodes.Clear()
            For Each row As DataRow In DirectoryTable.Rows
                Dim ANode As New TreeNode
                ANode.Name = "User." & row("ID")
                ANode.Text = row("UserName")
                ANode.Tag = "User"
                ANode.ImageIndex = 0 '人头像
                ANode.SelectedImageIndex = 0 '人头像
                TreeView.Nodes.Add(ANode)
            Next
            For Each Node As TreeNode In TreeView.Nodes
                EasyOnLine.GetFirstDirectoryTree(Integer.Parse(Node.Name.Split(".")(1)), Node.Nodes, True)
                '0级目录展开，1级目录折叠
                Node.Expand()
        ...
        End If
    End Sub
    Public Shared Sub GetFileTree(ByVal TreeNode As TreeNode, Nodes As TreeNodeCollection, ByVal ShowFile As Boolean, Optional ByVal ExpandTreeNode As Boolean = True, Optional ByVal ShowTreeFile As Boolean = False)
        Dim NNode As Integer = TreeNode.Nodes.Count
        Nodes.Clear()
        Dim IsExist As Boolean
        Dim TreeNodeName() As String = TreeNode.Name.Split(".")
        Dim DirectoryTable As DataTable = Nothing
        If TreeNodeName(0).ToUpper = "DIRECTORY" Then
            DirectoryTable = GetDataTable("Select * from Directorys where DirectoryID=" & TreeNodeName(1))
        End If
        If TreeNodeName(0).ToUpper = "USER" Then
            DirectoryTable = GetDataTable("Select * from Directorys where DirectoryID=0 and UserID=" & TreeNodeName(1))
        End If
        If Not IsNothing(DirectoryTable) Then
            For Each row As DataRow In DirectoryTable.Rows
                IsExist = False
                If TreeNode IsNot Nothing Then
                    For Each Node In TreeNode.Nodes
                        If Node.name = "Directory." & row("ID") Then
                            IsExist = True
                            Exit For
                        End If
                    Next
                ...
                ANode.Tag = "Directory"
                ANode.ImageIndex = 1 '文件夹
                ANode.SelectedImageIndex = 2 '人头像
        ...
        End If
        If TreeNodeName(0).ToUpper = "DIRECTORY" Then
            Dim FileTable As DataTable = GetDataTable("Select * from Files where DirectoryID=" & TreeNodeName(1)) '& " and (UserID=0 or (Visible=1 or UserID=" & CurrentUserID & "))"
            For Each row As DataRow In FileTable.Rows
                IsExist = False
                If TreeNode IsNot Nothing Then
                    For Each Node In TreeNode.Nodes
                        If Node.name = "File." & row("ID") Then
                            IsExist = True
                            Exit For
                        End If
                    Next
                End If
                Dim ANode As New TreeNode
                ANode.Name = "File." & row("ID")
                ANode.Text = row("FileName")
                ANode.ToolTipText = row("Note") & ",创建时间=" & row("CreateTime") & ",更新时间=" & row("ModifyTime")
                ANode.Tag = "File"
                ANode.ImageIndex = 3 '文件
                ANode.SelectedImageIndex = 3 '人头像
                If ShowFile Then Nodes.Add(ANode.Clone)
                If IsExist = False And ShowTreeFile Then
                    TreeNode.Nodes.Add(ANode)
                End If
            Next
        End If
        If NNode = 0 And ExpandTreeNode Then
            TreeNode.Expand()

        End If
    End Sub
    Public Shared Sub GetFirstDirectoryTree(ByVal UserID As Integer, ByVal Nodes As TreeNodeCollection, Optional ByVal ShowAllNodes As Boolean = False)
        Dim IsExist As Boolean
        Dim DirectoryTable As DataTable = GetDataTable("Select * from Directorys where  DirectoryID=0 and UserID=" & UserID)
        If Not IsNothing(DirectoryTable) Then
            For Each row As DataRow In DirectoryTable.Rows
                IsExist = False
                ...
                If IsExist = False Then
                    ...
                    If ShowAllNodes Then
                        GetFileTree(ANode, (New TreeNode).Nodes, False)
                    End If
                    ANode.Collapse()
                End If
            Next
    ...
    Public Shared Function GetFileData(ByVal FileID As Integer) As DataSet
        Dim Mydataset As DataSet = Nothing
        Dim FileTable As DataTable = GetDataTable("Select * from Files where ID=" & FileID)
        If Not IsNothing(FileTable) Then
            For Each row As DataRow In FileTable.Rows
                ...
                Dim Bytes() As Byte = UnGZip(row("FileData")）
                Dim ms As New MemoryStream(Bytes)
                Mydataset = bf.Deserialize(ms)
            Next
        End If
        Return Mydataset
    End Function
    Public Shared Sub GetFileNoteAndVisble(ByVal FileID As Integer, ByRef Visible As Integer, ByRef Note As String)
        Dim FileTable As DataTable = GetDataTable("Select * from Files where ID=" & FileID)
        If Not IsNothing(FileTable) Then
            If FileTable.Rows.Count > 0 Then
                Note = FileTable.Rows(0).Item("Note")
                Visible = FileTable.Rows(0).Item("Visible") - 1
            End If

        End If
    End Sub
    Public Shared Function CreateDirectory(ByVal UserID As Integer, ByVal DirectoryID As Integer, ByVal DirectoryName As String, ByVal CreateType As Integer) As Boolean
        On Error Resume Next
        Dim CNN As SqlConnection
        Dim ATable As DataTable = Nothing
        CNN = New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        Dim IsCreateDirectory As Boolean = False
        If CNN.State <> ConnectionState.Closed Then
            Dim DataAdapter As New SqlDataAdapter("Select * from Directorys", CNN)
            ATable = New DataTable
            DataAdapter.Fill(ATable)

            Select Case CreateType
                Case 1 '添加
                    Dim rows() As DataRow = ATable.Select("DirectoryName='" & DirectoryName & "' and DirectoryID=" & DirectoryID)
                    If rows.Length = 0 Then
                        Dim ACommand As New SqlCommand("insert into Directorys values(" & (ATable.Compute("Max(ID)", "") + 1) & ",'" & DirectoryName & "'," & DirectoryID & "," & UserID & ")", CNN)
                        ACommand.ExecuteNonQuery()
                        IsCreateDirectory = True
                    End If
                Case 2 '修改
                    Dim rows() As DataRow = ATable.Select("ID=" & DirectoryID)
                    If rows.Length = 1 Then
                        If rows(0).Item("UserID") = UserID Then
                            Dim FileDataAdapter As New SqlDataAdapter("Select * from Files where DirectoryID=" & DirectoryID, CNN)
                            Dim AFileTable As New DataTable
                            FileDataAdapter.Fill(AFileTable)
                            If AFileTable.Rows.Count > 0 Then
                                MsgBox("目录非空，不能删除")
                            Else
                                Dim ACommand As New SqlCommand("update Directorys set DirectoryName='" & DirectoryName & "'where ID=" & DirectoryID, CNN)
                                ACommand.ExecuteNonQuery()
                                IsCreateDirectory = True
                            End If
                        ...
                        Else
                            MsgBox("该目录非当前用户创建，不能修改")
                        End If
                    End If
                Case 3 '删除
                    Dim rows() As DataRow = ATable.Select("ID=" & DirectoryID & " and UserID=" & UserID)
                    If rows.Length = 1 Then
                        If rows(0).Item("UserID") = UserID Then
                            Dim ACommand As New SqlCommand("delete from Directorys where ID=" & DirectoryID, CNN)
                            ACommand.ExecuteNonQuery()
                            IsCreateDirectory = True
                        Else
                            MsgBox("该目录非当前用户创建，不能删除")
                        End If
                    End If
            End Select
    ...
    End Function
    Public Shared Function DeleteFile(ByVal FileID As Integer, UserID As Integer) As Boolean
        On Error Resume Next
        Dim CNN As SqlConnection
        Dim ATable As DataTable = Nothing
        CNN = New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        If CNN.State <> ConnectionState.Closed Then
            Dim FileDataAdapter As New SqlDataAdapter("Select * from Files where ID=" & FileID, CNN)
            Dim AFileTable As New DataTable
            FileDataAdapter.Fill(AFileTable)
            If AFileTable.Rows.Count > 0 Then
                If AFileTable.Rows(0).Item("UserID") <> UserID Then
                    MsgBox("非当前用户创建，不能删除")
                End If
                Dim ACommand As New SqlCommand("delete from Files where ID=" & FileID & " and UserID=" & UserID, CNN)
                ACommand.ExecuteNonQuery()
            End If
            CNN.Close()
        End If
        Return True
    End Function
    Public Shared Function ModifyFile(ByVal FileID As Integer, ByVal FileName As String, ByVal Note As String, VisibleType As Integer) As Boolean
        On Error Resume Next
        Dim CNN As SqlConnection
        Dim ATable As DataTable = Nothing
        Dim IsDeleteFile As Boolean = False
        CNN = New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        If CNN.State <> ConnectionState.Closed Then
            Dim ACommand As New SqlCommand("update Files set FileName='" & FileName & "',Note='" & Note & "',Visible=" & VisibleType & " where ID=" & FileID, CNN)
            ACommand.ExecuteNonQuery()
            IsDeleteFile = True
            CNN.Close()
        End If
        Return True
    End Function
    Public Shared Function UpdateFileData(ByVal Mydataset As DataSet, ByVal UserID As Integer, ByVal DirectoryID As Integer, ByVal FileName As String, ByVal Note As String, ByVal VisibleType As Integer, Nodes As TreeNodeCollection) As Boolean
        On Error Resume Next
        Dim CNN As SqlConnection
        Dim ATable As DataTable = Nothing
        CNN = New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        If CNN.State <> ConnectionState.Closed Then
            Dim DataAdapter As New SqlDataAdapter("Select * from Files", CNN)
            Dim builder As New SqlCommandBuilder(DataAdapter)
            ATable = New DataTable
            DataAdapter.Fill(ATable)
            If ATable.Columns.Count > 0 Then
                Dim rows() As DataRow = ATable.Select("DirectoryID=" & DirectoryID & " and FileName='" & FileName & "'")
                Dim Row As DataRow
                ...
                If rows.Length = 0 Then
                    ...
                    Row("CreateTime") = Now
                Else
                    Dim myMsgBoxResult As MsgBoxResult = MsgBox("同名文件已经存在，覆盖当前数据", MsgBoxStyle.YesNo, "覆盖数据")
                    If myMsgBoxResult = MsgBoxResult.No Then
                        Return False
                    End If
                    Row = rows(0)
                End If
                ...
                Row("Note") = Note
                Row("UserID") = UserID
                Row("Visible") = VisibleType
                Row("ModifyTime") = Now
                Dim bf As System.Runtime.Serialization.Formatters.Binary.BinaryFormatter = New System.Runtime.Serialization.Formatters.Binary.BinaryFormatter()
                Dim ms As New MemoryStream()
                Mydataset.RemotingFormat = SerializationFormat.Binary
                bf.Serialize(ms, Mydataset)
                Dim FileArray() As Byte = CompressGZip(ms.ToArray) '上传压缩数据
                Row("FileData") = FileArray '上传压缩数据
                Row("FileSize") = FileArray.Length / 1024
                DataAdapter.Update(ATable)
            ...
            Else
                CNN.Close()
                MsgBox("上传失败!,请检查数据库连接设置。", MsgBoxStyle.Critical)
                Return False
            End If
    ...
    End Function
    Shared Function AddUser(ByVal LoginName As String, ByVal Password As String, ByVal UserName As String, ByVal Department As String, Phone As String) As Boolean
        Dim CNN As New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        If CNN.State = ConnectionState.Open Then
            Dim DataAdapter As New SqlDataAdapter("Select * from Users where id>0", CNN)
            ...
            End If
            ATable = New DataTable
            DataAdapter = New SqlDataAdapter("Select * from Users where LoginName='" & LoginName & "'", CNN)
            DataAdapter.Fill(ATable)
            If ATable.Rows.Count > 0 Then
                MsgBox（"登录名已经存在"）
            Else
                Dim ACommand = New SqlCommand("insert into  Users values(" & ID & ",'" & LoginName & "','" & Password & "','" & UserName & "','" & Department & "','" & Phone & "',1)", CNN)
                ACommand.ExecuteNonQuery()
                CNN.Close()
                Return True
            End If
        ...
        Return False
    End Function
    Shared Function DeleteUser(ByVal UserID As Integer) As Boolean
        Dim CNN As New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        If CNN.State = ConnectionState.Open Then
            Dim ACommand As New SqlCommand("delete from Users where ID=" & UserID, CNN)
            ACommand.ExecuteNonQuery()
            CNN.Close()
            Return True
        End If
        Return False
    End Function
    Shared Function ModifyUser(ByVal UserID As Integer, ByVal LoginName As String, ByVal Password As String, ByVal UserName As String, ByVal Department As String, Phone As String, Valid As Boolean) As Boolean
        Dim CNN As New SqlConnection(OnLineConnectString) '连接到数据库
        CNN.Open()
        If CNN.State = ConnectionState.Open Then
            Dim ACommand As New SqlCommand
            ACommand = New SqlCommand("update Users set LoginName='" & LoginName & "',Password='" & Password & "',UserName='" & UserName & "',Department='" & Department & "',Phone='" & Phone & "',Valid=" & If(Valid, "1", "0") & " where ID=" & UserID, CNN)
            ACommand.ExecuteNonQuery()
            CNN.Close()
            Return True
        End If
    ...
    Public Shared Sub CreateDataBase()
        ...
        If CNN.State = ConnectionState.Open Then
            ...
            Dim ATable As DataTable
            ATable = GetDataTable("Select  * FROM dbo.SysObjects WHERE ID = object_id(N'Users') AND OBJECTPROPERTY(ID, 'IsTable') = 1")
            If ATable.Rows.Count = 0 Then
                ACommand = New SqlCommand("Create Table Users(ID int primary key,LoginName varchar(Max),PassWord varchar(Max),UserName varchar(Max),Department varchar(Max),Phone varchar(Max),AllowAdd Bit,AllowModify Bit,AllowDelete,Valid Bit)", CNN)
                ACommand.ExecuteNonQuery()
                ACommand = New SqlCommand("insert into Users values(0,'Master','123456','管理员','管理部门','000-000',1,1,1,1)", CNN)
                ACommand.ExecuteNonQuery()
            End If

            ATable = GetDataTable("SELECT  * FROM dbo.SysObjects WHERE ID = object_id(N'Directorys') AND OBJECTPROPERTY(ID, 'IsTable') = 1")
            If ATable.Rows.Count = 0 Then
                ACommand = New SqlCommand("Create Table Directorys(ID int primary key,DirectoryName varchar(Max),DirectoryID int,UserID int)", CNN)
                ACommand.ExecuteNonQuery()
                ACommand = New SqlCommand("insert into Directorys values(1,'公共目录',1,0)", CNN)
                ACommand.ExecuteNonQuery()
            End If

            ATable = GetDataTable("SELECT  * FROM dbo.SysObjects WHERE ID = object_id(N'Files') AND OBJECTPROPERTY(ID, 'IsTable') = 1")
            If ATable.Rows.Count = 0 Then
                ACommand = New SqlCommand("Create Table Files(ID int primary key,FileName varchar(Max),Note varchar(Max),Visible int,DirectoryID int,FileData varbinary(Max),CreateTime datetime,ModifyTime datetime,UserID int)", CNN)
                ACommand.ExecuteNonQuery()
            End If
            '创建一个项目公共目录
    ...
    Public Shared Function CompressGZip(ByVal rawData As Byte()) As Byte()
        Dim ms As MemoryStream = New MemoryStream()
        Dim compressedzipStream As GZipOutputStream = New GZipOutputStream(ms)
        compressedzipStream.Write(rawData, 0, rawData.Length)
        compressedzipStream.Close()
        Return ms.ToArray()
    End Function

    Public Shared Function UnGZip(ByVal byteArray As Byte()) As Byte()
        Dim gzi As GZipInputStream = New GZipInputStream(New MemoryStream(byteArray))
        Dim re As MemoryStream = New MemoryStream(50000)
        Dim count As Integer = 1
        Dim data As Byte() = New Byte(49999) {}

        While True
            count = gzi.Read(data, 0, data.Length)
            If count <> 0 Then
        ...
        Dim overarr As Byte() = re.ToArray()
        Return overarr
    End Function
