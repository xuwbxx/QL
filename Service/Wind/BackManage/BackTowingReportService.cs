...
{
    public class BackTowingReportService
    {
        private readonly CloudWind_KingBase_UnitOfWorkFactory _cloudWindUowFactory;

        public BackTowingReportService(CloudWind_KingBase_UnitOfWorkFactory cloudWindUowFactory)
        {
        ...
        public (List<CloudWindManageTaskInfo> list, int totalCount, int pageIndex, string msg) ListQuery(CloudWindBackManageRequest request)
        {
            ...
            try
            {
                using (var uow = _cloudWindUowFactory.Create())
                {
                    ...
                    predicate = PredicateBuilder.And(predicate, a => a.SoftwareID == (int)CloudWindSoftware.拖航可行性分析与预报);
                    ...
                    {
                        list.Add(new CloudWindManageTaskInfo()
                        {
                            ID = a.id,
            ...
            {
                LoggerUtils.Error(ex.ToString(), typeof(BackTowingReportService));
                msg = "发生错误";
            }
